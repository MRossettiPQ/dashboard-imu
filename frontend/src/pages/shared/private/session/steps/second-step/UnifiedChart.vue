...
const measurementCount = computed(() =>
  sensors.value.reduce((sum, s) => sum + (s.measurementsArray?.length ?? 0), 0),
);
...
watch(
  sensors,
  async () => {
    const count = sensors.value.reduce((sum, s) => sum + (s.measurementsArray?.length ?? 0), 0);
    if (count > 0) {
      await nextTick();
      throttledSetOption();
    }
...
function setOption() {
  ...
  for (const sensor of sensors.value) {
    if (!sensor.measurements || sensor.measurementsArray.length === 0) continue;
    ...
    for (const col of props.allowedColumn) {
      ...
      const numericData = _.map(sensor.measurementsArray, col).map((val) => {
