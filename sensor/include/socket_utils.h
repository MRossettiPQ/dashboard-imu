...
inline void socketIOEvent(const socketIOmessageType_t type, uint8_t* payload, size_t length) {
	...
	case sIOtype_CONNECT: {
		...
		const String name = ap_ssid;
		const String mac = WiFi.macAddress();

		JsonDocument registerSensor;

		registerSensor["origin"] = "SENSOR";
		registerSensor["type"] = "SENSOR_SERVER_REGISTER_SENSOR";
		registerSensor["originIdentifier"] = WiFi.macAddress();

		auto content = registerSensor["content"].to<JsonObject>();
		content["ip"] = ip;
		content["name"] = name;
		content["mac"] = mac;
		content["type"] = "GYROSCOPE";

		String registerPayload;
		serializeJson(registerSensor, registerPayload);

		socketIO.sendEVENT("[\"SENSOR_SERVER_REGISTER_SENSOR\"," + registerPayload + "]");
		break;
	}
